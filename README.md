...
Точкой входа в программу является класс ***[Main.java](src/tracker/Main.java)***.

Проект состоит из следующих классов:

- ***Main*** - точка входа
- ***tracker.methods.MenuMethods*** - класс содержит методы для работы основного и вложенных меню.
- ***tracker.methods.FileMethods*** - класс содержит методы для работы с файлами данных.
- ***tracker.model.Record*** - класс описывает объект типа Record и содержит метода для работы с
  объектом
- ***tracker.methods.RecordMethods*** - класс содержит методы для работы с таблицами данных типа
  Record
- ***tracker.comparator.RecordDateCategoryAmountComparator*** - класс содержит компаратор для
  сортировки записей
- ***tracker.methods.BudgetMethods*** - класс содержит методы для построения диаграмм
- ***tracker.methods.DateMethods*** - класс содержит методы для работы с датами
- ***tracker.methods.Guide*** - класс содержит методы для работы со справочниками доходов и
  расходов.
- ***tracker.service.Colors*** - вспомогательный класс, содержит константы для работы с цветом.
- ***tracker.tests.RecordTests*** - тесты класса Record для некоторых значений и для компаратора.
...
при совпадении категории - по сумме). 

Если по расходам в текущем месяце не было записей программа сообщит об этом. 

В нижнем меню можно выбрать другой месяц или выйти в подменю работы с таблицей данных.
...
Если по доходам в текущем месяце не было записей программа сообщит об этом. 
...
и доходов по категориям. Данные диаграммы выводятся в процентном соотношении, за 100% принята сумма
всех доходов.

Если по доходам или расходам в текущем месяце не было записей программа выводит данные только в евро,
для расчета масштаба диаграммы за 100% принимается сумма доходов или расходов.

Если в текущем месяце не было записей программа сообщит об этом.
...
категорию и выводит на экран выборку из основной таблицы по этой категории за текущий месяц. 

Если по этой категории в текущем месяце не было записей программа сообщит об этом. 
